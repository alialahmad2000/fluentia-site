...
export default function ApplyForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" dir="rtl" noValidate>
      ...
      <Field label="لماذا تريد الانضمام لبرنامج الشركاء؟" required error={errors.why_join}>
        <textarea
          ...
          className={[
            ...
            'border focus:outline-none focus:ring-2 focus:ring-amber-400/40',
            errors.why_join
              ? 'border-red-400/50'
      ...
      {/* heard_from — custom dropdown, never native select */}
      ...
      {errors.terms_accepted && (
        <p className="text-xs text-red-300 text-right -mt-3">{errors.terms_accepted}</p>
      )}
      ...
      {errors._general && (
        ...
        >
          ...
          <p className="text-sm flex-1 text-right" style={{ color: '#fecaca' }}>{errors._general}</p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full py-4 rounded-xl font-bold text-lg transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        style={{
